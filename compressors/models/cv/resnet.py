...
class ResNet(BaseDistilModel):
    ...
    def forward(
        ...
        x, f0_pre = self.layer1(x)
        f0 = x
        x, f1_pre = self.layer2(x)
        f1 = x
        x, f2_pre = self.layer3(x)
        f2 = x
        x, f3_pre = self.layer4(x)
        f3 = x
        ...
        if output_hidden_states:
            if preact:
                hiddens = tuple([f0_pre, f1_pre, f2_pre, f3_pre, f4])
            else:
